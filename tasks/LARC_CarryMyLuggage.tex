...
\begin{enumerate}[nosep]
	\item \textbf{Picking up the bag:} The robot picks up the bag pointed at by the operator, or asks them to handle it to the robot.
	\item \textbf{Following the operator:} The robot should inform the operator when it is ready to follow them. The operator walks slowly towards the car; after reaching the car, the operator takes the bag back and thanks the robot.
	\item \textbf{Obstacles:} The robot will face 2 obstacles along its way (in arbitrary order):
	\begin{enumerate*}[label=(\alph*)]
	    \item a random object on the ground,
% 		\item a small object on the ground,
% 		\item a hard-to-see object,
		\item a crowd of 3 to 4 people obstructing the path outside.
