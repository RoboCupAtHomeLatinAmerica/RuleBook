...
A \RoboCup\AtHome{} competition consists of two sub-leagues with the following stages:

\begin{enumerate}
	\item \textbf{Poster Section:} Scientific presentation made by the team during \SetupDays{}.
 	All registered teams can participate.

	\item \textbf{\RobotInspection{}:} For security, robots are inspected during \SetupDays{}.
	All registered teams must participate.

	\item \textbf{Sub-league B:} Four tasks, assessing the robot's basic abilities.
 	Only teams that passed the \RobotInspection{} and competes in sub-league B can participate.

	\item \textbf{Sub-league A:} Four tasks, assessing more complex abilities and behaviors.
 	Only teams that passed the \RobotInspection{} and competes in sub-league A can participate.

	\item \textbf{Restaurant:} A restaurant task that takes place outside the arena.

	\item \textbf{\FINAL{}:} An open demonstration, asking teams to showcase complex behaviors and novel approaches. There will be a final for each sub-league.
% 	The two best scoring teams (\SONE{} and \STWO{} combined) can participate.
\end{enumerate}
...
\begin{table}[h]
	...
	\begin{center}
		\begin{tabularx}{14.56cm}{S|S|S|S|S|S|S|S}
			...
			\multicolumn{2}{M|}{ \cellcolor[HTML]{67FD9A}\iterm{Sub-league~A Sub-league~B}} &
			\multicolumn{2}{M|}{ \cellcolor[HTML]{9698ED}\iterm{Restaurant}} &
...
During the meeting, teams can ask questions and discuss the upcoming tests with the \abb{TC} and \abb{OC}. 
The starting time will be announced by the \abb{OC}. 
Decisions made in the \TLM{} are binding. 
The \abb{TC} and referees on site will decide on anything coming up during or after a test.

\subsection{Scoring System}
\label{sec:rules:scoringsystem}
Each test has a main objective and a set of bonuses.
% To score in a test, a team must accomplish the main goal (in parts if allowed). 
...
% The final score is calculated differently and is normalized (see~\ref{sec:finals:scoring}). 
